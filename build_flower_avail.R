...
AvailFlwMat <- function(years){
  ...
  C.data <- BuildPPIMat(mydata$int.data, FALSE)
  ...
  for(y in years) {
    ...
    for(m in meadows) {
      ...
      for(w in watches) {
        #cat(c("Year", y, "Meadow", m, "Watche", w, "\n"))
        ...
        count <- count + 1 	 # total number of meadow-watches
      }
    }
  }
